function [deltaV] = check_analitic(vin, vpl, mu, rp, alpha, figure_number, colors, color_number, lines, rp_arr)
...
plot(x_arr, y_arr, lines(color_number), 'Color', colors(color_number));
...
x_a = []
for el = y_a
    x_a(end + 1) = a * sqrt(1 + el^2/rp^2) * 0.8 - 11000;
end
plot(x_a, y_a, lines(color_number), 'Color', colors(2));

legend("Позиция планеты", "Численное моделироание", "Аналитическое решение")
title("Земля угол между скоростями " + num2str(alpha * 180 / pi) + " прицельное расстояние " + num2str(rp))